...
class DataHandlerHook
{
        ...
        foreach ($this->tcaHelper->getColumnsWithFileReferences($table) as $column) {
            ...
            foreach ($sysFileReferenceRecords as $sysFileReferenceUid => $sysFileReferenceRecord) {
                ...
                $mergedCropVariants = $this->tcaHelper->getMergedCropVariants(
                    ...
                    $column,
                    $pageUid,
                    $this->tcaHelper->getTypeOfRecord($this->getCurrentFullRecord($dataHandler), $table)
                );

                // We need at least 2 CropVariants. With just 1 there is no target to copy something over ;-)
                $cropVariants = json_decode((string)$sysFileReferenceRecord['crop'], true, 512, JSON_THROW_ON_ERROR) ?? [];
                if (is_array($cropVariants) && count($cropVariants) > 1) {
                    ...
                    if ($this->isValidCropVariant($firstCropVariant)) {
                        ...
                        $dataHandler->datamap['sys_file_reference'][$sysFileReferenceUid]['crop']
                            = json_encode($cropVariants, JSON_THROW_ON_ERROR);
                    }
                }
            }
        }
    ...
    /**
     * Returns Datamap for a given table.
     * It contains all records to store or modify.
     */
    protected function getDatamapForTable(string $table, DataHandler $dataHandler): array
    {
        return array_key_exists($table, $dataHandler->datamap) ? $dataHandler->datamap[$table] : [];
    }
    ...
    protected function isSelectedRatioAvailableInForeignCropVariant(
        array $mergedCropVariants,
        string $cropVariantName,
        string $selectedRatio
    ): bool {
        return in_array(
            $selectedRatio,
            $this->getAllowedAspectRatiosForCropVariant($mergedCropVariants, $cropVariantName),
            true
        );
    }
     ...
     */
    protected function getAllowedAspectRatiosForCropVariant(
        array $mergedCropVariants,
        string $cropVariantName
    ): array {
        if (!array_key_exists($cropVariantName, $mergedCropVariants)) {
            return [];
        }

        if (!array_key_exists('allowedAspectRatios', $mergedCropVariants[$cropVariantName])) {
            return [];
        }

        if (!is_array($mergedCropVariants[$cropVariantName]['allowedAspectRatios'])) {
            return [];
        }

        return array_keys($mergedCropVariants[$cropVariantName]['allowedAspectRatios']);
    }
